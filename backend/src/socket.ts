...
export class SocketServer {
  ...
  private sockets: Map<string, { socketId: string; userId?: string | number; role?: string; usuario?: string }> = new Map();
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('private_message', (data: { to: string; room?: string; content?: string }) => {
        try {
          ...
          if (this.sockets.has(to)) targetSocketId = to; 
          else {
      ...
      socket.on('leave_room', (payload: any) => {
        try {
          // CORRECCIÓN 3: Inicializamos room también aquí
          let room: string = "";

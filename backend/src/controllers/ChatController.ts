...
export class ChatController {
  ...
  async enviarPictograma(chatId: number, emisorId: number, pictogramaId: number) {
    console.log(`(API) POST /chats/${chatId}/mensajes body={emisorId:${emisorId}, pictogramaId:${pictogramaId}}`);
    const msg = await this.facade.enviarPictograma(chatId, emisorId, pictogramaId);
    console.log("(API) 200 OK ->", msg, "\n");
    return msg;
  }

  // Simula: GET /chats/:chatId/mensajes
  async listarMensajes(chatId: number) {
    console.log(`(API) GET /chats/${chatId}/mensajes`);
    const list = await this.facade.listarMensajes(chatId);
